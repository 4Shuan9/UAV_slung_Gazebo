...
def process_error_plot(file_path):
    ...
    def plot_sub_err(ax, cv_t, err_cv, comp_t, err_comp, axis_name, title):
        ...
        ax.text(TIME_RANGE[0] + 0.5, max_err_raw + 0.01, f'{max_err_raw:.2f}', color='#1f77b4', va='bottom', ha='left', fontsize=12, fontweight='bold')
        ax.text(TIME_RANGE[0] + 0.5, min_err_raw - 0.02, f'{min_err_raw:.2f}', color='#1f77b4', va='top', ha='left', fontsize=12, fontweight='bold')

        # --- Comp Error 峰值辅助线与标签 (红色) ---
        ...
        # 将 Comp 的文字标签向右偏移 5s，避免重叠
        ax.text(TIME_RANGE[0] + 5.0, max_err_comp + 0.01, f'{max_err_comp:.2f}', color='#d62728', va='bottom', ha='left', fontsize=12, fontweight='bold')
        ax.text(TIME_RANGE[0] + 5.0, min_err_comp - 0.02, f'{min_err_comp:.2f}', color='#d62728', va='top', ha='left', fontsize=12, fontweight='bold')
